...
async def ddl_call_back(bot, update):
    logger.info(update)
    cb_data = update.data
    tg_send_type, youtube_dl_format, youtube_dl_ext, randem = cb_data.split("=")
    ...
    if tg_send_type != "file" or youtube_dl_format != "LFO" or youtube_dl_ext != "NONE":
        command_to_exec = [
            "yt-dlp",
            "-c",
        ...
        with open(file_name, "wb") as f_handle:
            while True:
                ...
                if round(diff % 5.00) == 0 or downloaded >= total_length:
                    ...
                    speed = downloaded / diff if diff else 0
                    elapsed_time = round(diff) * 1000
                    time_to_completion = round((total_length - downloaded) / speed) * 1000 if speed else 0
